...
+ [Blue/Green Deployment with CodeDeploy](#deployment-type-bluegreen)
...
The **blue/green** deployment type uses the blue/green deployment model controlled by CodeDeploy\. This deployment type allows you to verify a new deployment of a service before sending production traffic to it\. For more information, see [What Is CodeDeploy?](https://docs.aws.amazon.com/codedeploy/latest/userguide/welcome.html) in the *AWS CodeDeploy User Guide*\.
...
+ When an CodeDeploy application and deployment group are initially created, the following must be specified:
...
+ When an CodeDeploy deployment has been created for an Amazon ECS service, CodeDeploy creates a *replacement task set* \(or *green task set*\) in the deployment\. If a test listener was added to the load balancer, CodeDeploy routes your test traffic to the replacement task set\. This is when any validation tests can be run\. Then, CodeDeploy reroutes the production traffic from the original task set to the replacement task set according to the traffic rerouting settings for the deployment group\.
...
When an Amazon ECS service using the blue/green deployment type is initially created using the Amazon ECS console, an Amazon ECS task set and the following CodeDeploy resources are created automatically with the following default settings\. 
...
|  Traffic rerouting settings  |  Traffic rerouting – The default is set to **Reroute traffic immediately**\. It can be changed in the CodeDeploy console or by updating the `TrafficRoutingConfig`\. For more information, see [CreateDeploymentConfig](https://docs.aws.amazon.com/codedeploy/latest/APIReference/API_CreateDeploymentConfig.html) in the *AWS CodeDeploy API Reference*\.  | 
