...
|  `--target-groups`  |  Specifies one or more target groups to be registered with the service\. Specify the Amazon Resource Name \(ARN\) of the target group, the container name and the container port for each group to register\. A single `--targetgroups` flag specifies only one target group\. Add additional `--target-groups` flags to add additional target groups\. Type: Strings For more information see [Registering multiple target groups with a service](register-multiple-targetgroups.md)\. Required: No  | 
