...
|  `--target-groups`  |  Specifies one or more target groups to be registered with the service\. Specify the Amazon Resource Name \(ARN\) of the target group, the container name and the container port for each group to register\. A single `--target-groups` flag specifies only one target group\. Add additional `--target-groups` flags to add additional target groups\. You use a target group to configure where aload balancer direct traffic to, for example Amazon EC2 instances\. When you create a load balancer, you create one or more listeners and configure listener rules to direct the traffic to one target group\. Type: Strings For more information see [Registering multiple target groups with a service](register-multiple-targetgroups.md)\. Required: No  | 
