...
+ For services that use an Application Load Balancer or Network Load Balancer, there is a limit of five target groups you can attach to a service\.
...
+ If you configure your Application Load Balancer to use slow start mode, you must configure your container health check to return an `UNHEALTHY` status until after the slow start period has ended and also configure the health check grace period value for your Amazon ECS service to the same duration as the slow start mode duration\. This will ensure that the service scheduler will ignore unhealthy container health checks until your tasks have warmed up and are ready to receive traffic\. For more information about slow start mode, see [Target Groups for Your Application Load Balancers](https://docs.aws.amazon.com/elasticloadbalancing/latest/application/load-balancer-target-groups.html)\.
